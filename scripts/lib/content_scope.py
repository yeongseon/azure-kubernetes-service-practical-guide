...
# ``docs/tutorials/``, ``docs/workload-guides/``, ``docs/visualization/``,
# ``docs/meta/``, ``docs/reference/``, ``docs/contributing/``,
# ``docs/index.md``) are out of scope regardless of their name.
...
# ``troubleshooting/methodology/index.md`` and
# ``troubleshooting/evidence-packs/index.md``, both of which already carry
# ``content_validation`` blocks) are intentionally NOT in this list -- they
...
def is_in_scope(rel_path: Union[Path, str]) -> bool:
    ...
    >>> from pathlib import Path
    >>> is_in_scope("platform/cluster-architecture.md")
    True
    >>> is_in_scope("troubleshooting/methodology/index.md")  # factual subsection landing
    True
    >>> is_in_scope("troubleshooting/evidence-packs/index.md")  # factual subsection landing
    True
    >>> is_in_scope("platform/index.md")  # navigation
    False
    >>> is_in_scope("troubleshooting/playbooks/dns-resolution-failure.md")
    True
    >>> is_in_scope("troubleshooting/kql/nodes/index.md")  # excluded subpath
    False
    >>> is_in_scope("troubleshooting/lab-guides/node-pressure.md")
    False
    >>> is_in_scope("workload-guides/stateful-workloads.md")  # outside sections
    False
...
def is_tautological_text(text: object) -> bool:
    ...
    True
    >>> is_tautological_text("AKS supports cluster autoscaler and node pool scaling")
    False
